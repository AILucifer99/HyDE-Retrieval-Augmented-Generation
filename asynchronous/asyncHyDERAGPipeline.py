...
# ----------------------------
# HyDERAG: Hybrid RAG system with multi-LLM support
# ----------------------------
class HyDERAG:
    """
    ...
    # Initializes embeddings and LLMs with defaults if none are provided
    ...
    def answer_question(self, 
        ...
        try:
            ...
            # Log final response generation step
...
# ----------------------------
# AsyncHyDERAG: Asynchronous version of HyDERAG for improved concurrency
# ----------------------------
class AsyncHyDERAG(HyDERAG):
    ...
    def _initialize_models(self):
        ...
        groq_model = self.hyde_llm_model_name or default_models["groq"]
        self.llm_models["groq"] = ChatGroq(
            model=groq_model,
            temperature=self.temperature,
            max_tokens=self.hyde_max_tokens,
        ...
        openai_model = self.answer_llm_model_name or default_models["openai"]
        self.llm_models["openai"] = ChatOpenAI(
            model=openai_model,
            max_tokens=self.answer_max_tokens,
            temperature=self.temperature,
        ...
        google_model = self.answer_llm_model_name or default_models["google"]
        self.llm_models["google"] = ChatGoogleGenerativeAI(
            model=google_model,
            temperature=self.temperature,
            max_output_tokens=self.answer_max_tokens,
        ...
        self.hyde_llm = self.llm_models[self.hyde_llm_provider]
        self.answer_llm = self.llm_models[self.answer_llm_provider]


    # Asynchronously generates a hypothetical answer using HyDE prompt
    async def _generate_hypothetical_answer(self, question: str, custom_template: Optional[str] = None) -> str:
        hyde_template = custom_template or """You are an English expert mastering in creating hypothetical answers. 
        For the given user question generate a hypothetical answer. 
        Do not generate anything else just the answer. The question that you need to answer is: 
        Question: {user_question}
        ...
        prompt = ChatPromptTemplate.from_template(hyde_template)
        formatted_prompt = prompt.format(user_question=question)
        self.logger.info("Generating hypothetical answer (async)")
        response = await self.hyde_llm.ainvoke(formatted_prompt)
        return response.content


    # Main async method to answer a question using the RAG pipeline
    async def answer_question(self, 
        ...
        original_temp = getattr(self.answer_llm, "temperature", None)
        original_max_tokens = getattr(self.answer_llm, "max_tokens", None)
        if temperature: self.answer_llm.temperature = temperature
        if max_tokens: self.answer_llm.max_tokens = max_tokens
        ...
        try:
            if disable_hyde:
                self.logger.info("Using direct retrieval (HyDE disabled)")
                similar_documents = await self.retriever.ainvoke(question)
            else:
                self.logger.info("Using HyDE approach for retrieval")
                hypothetical_answer = await self._generate_hypothetical_answer(question, hyde_template)
                similar_documents = await self.retriever.ainvoke(hypothetical_answer)
            ...
            # Log final response generation step
            self.logger.info("Generating final answer (async)")
            response = await self.answer_llm.ainvoke(final_query)
            return response.content
            ...
            if temperature: self.answer_llm.temperature = original_temp
            if max_tokens: self.answer_llm.max_tokens = original_max_tokens
            if retrieval_k is not None:
                self.retriever.search_kwargs["k"] = search_k


    # Runs both HyDE and direct retrieval in parallel and compares the responses
    async def compare_approaches(self, 
                                 ...
                                 answer_template: Optional[str] = None,
                                 retrieval_k: Optional[int] = None) -> Dict[str, str]:
        hyde_task = self.answer_question(
            question, data_path=data_path, disable_hyde=False,
            hyde_template=hyde_template, answer_template=answer_template,
            retrieval_k=retrieval_k
        )
        direct_task = self.answer_question(
            question, data_path=data_path, disable_hyde=True,
            answer_template=answer_template, retrieval_k=retrieval_k
        )

        hyde_answer, direct_answer = await asyncio.gather(hyde_task, direct_task)
